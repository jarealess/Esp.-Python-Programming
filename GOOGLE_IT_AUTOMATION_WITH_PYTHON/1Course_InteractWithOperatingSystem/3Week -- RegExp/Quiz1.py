...
#  characters (which includes letters, numbers, and underscores), as well as periods, dashes, and a plus sign, followed by a period 
# and a character-only top-level domain such as ".com", ".info", ".edu", etc. Fill in the regular expression to do that, using escape 
...
def check_time(text):
  pattern = r'^[1-9][1-2]?:[0-5][0-9][ ]?[amAM|pmPM]'
  result = re.search(pattern, text)
  return result != None
...
#  but not always, followed by a dash with 4 more digits. The zip code needs to be preceded by at least one space, and cannot be at the
#  start of the text.
print('\n'+'---'*20)
